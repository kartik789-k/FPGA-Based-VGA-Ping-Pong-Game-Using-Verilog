...
module pong_text(
    ...
    reg [3:0] row_addr; //row bit
    wire [3:0] row_addr_s, row_addr_l, row_addr_r, row_addr_o;
    reg [2:0] bit_addr; //column bit
   ...
   always @*
    ...
    //Which text should appear at current pixel (x,y)?
    //MUX (Multiplexer)
...
*/
    ...
    assign text_on = {score_on, logo_on, rule_on, over_on}; //This combines 4 signals into one 4-bit bus.
    //Which text layer is active right now?
    
    // ascii ROM interface
    assign rom_addr = {char_addr, row_addr};
    assign ascii_bit = ascii_word[~bit_addr];//This selects ONE pixel from 8-bit row.
